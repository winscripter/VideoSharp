...
public class VideoContext
{
    ...
    private void InitializeNoWait()
    {
        if (!_initialized)
        {
            ...
            var procarg = $"-v 0 -select_streams v:0 -count_frames -show_entries stream=nb_read_frames -of default=nokey=1:noprint_wrappers=1 \"{_path}\"";

            _fps = int.Parse(Video.LaunchAndRedirectFFprobeOutput(procarg));
    ...
    private async Task InitializeAsync()
    {
        if (!_initialized)
        {
            ...
            _fps = int.Parse(await Video.LaunchAndRedirectFFprobeOutputAsync(procarg));
