...
public static partial class Video
{
    ...
    /// <summary>
    /// Gets the amount of Frames Per Second (FPS) from a video, asynchronously.
    /// </summary>
    /// <param name="video">Input video file</param>
    /// <returns>Amount of FPS in a floating-point integer representation</returns>
    public static async Task<float> GetFpsCountAsync(string video)
    {
        EnsureFFprobeExists();
        EnsureVideoExists(video, nameof(video));
        ...
        var output = await LaunchAndRedirectFFprobeOutputAsync(msg);
